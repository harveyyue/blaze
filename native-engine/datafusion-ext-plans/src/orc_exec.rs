...
impl ExecutionPlan for OrcExec {
    ...
    fn execute(
        ...
        let opener = OrcOpener {
            ...
            _table_schema: self.base_config.file_schema.clone(),
...
impl FileOpener for OrcOpener {
    fn open(&self, file_meta: FileMeta) -> Result<FileOpenFuture> {
        let reader = OrcFileReaderRef(Arc::new(InternalReader {
            fs_provider: self.fs_provider.clone(),
            meta: file_meta.object_meta.clone(),
            input: OnceCell::new(),
        }));
...
impl InternalReader {
    ...
    pub(crate) fn read_fully(&self, range: Range<usize>) -> Result<Bytes> {
